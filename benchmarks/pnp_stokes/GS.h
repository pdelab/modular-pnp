...
INT electrokinetic_block_guass_seidel (
  ...
  dolfin::EigenVector pnp_rhs, stokes_rhs;
  ...
  while ( (iteration_count < max_bgs_it) && (relative_residual > relative_residual_tol) ){
    ...
    dolfin::Function d4 = (*dStokes)[0];
    dolfin::Function d5 = (*dStokes)[1];
    // *(dU->vector()) = *(d4.vector());
    // *(dPressure->vector()) = *(d5.vector());
    // *(dCation->vector()) = *(d1.vector());
    // *(dAnion->vector()) = *(d2.vector());
    // *(dPhi->vector()) = *(d3.vector());
    ...
    dolfin::Function d1 = (*dPNP)[0];
    dolfin::Function d2 = (*dPNP)[1];
    dolfin::Function d3 = (*dPNP)[2];
    *(dCation->vector()) = *(d1.vector());
    *(dAnion->vector()) = *(d2.vector());
    *(dPhi->vector()) = *(d3.vector());
    *(dU->vector()) = *(d4.vector());
