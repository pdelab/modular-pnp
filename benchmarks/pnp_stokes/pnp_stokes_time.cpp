...
int main(int argc, char** argv)
{
  ...
  auto dt = std::make_shared<Constant>(time_step_size);


  // interpolate
  auto V= std::make_shared<pnp_stokes_time::FunctionSpace>(mesh);
  auto SolutionFunction = std::make_shared<Function>(V);
  auto SolutionCation = std::make_shared<Function>((*SolutionFunction)[0]);
  auto SolutionAnion = std::make_shared<Function>((*SolutionFunction)[1]);
  auto SolutionPotential = std::make_shared<Function>((*SolutionFunction)[2]);
  auto SolutionVelocity = std::make_shared<Function>((*SolutionFunction)[3]);
  auto SolutionPressure = std::make_shared<Function>((*SolutionFunction)[4]);
  auto one_vec3 = std::make_shared<Constant>(1.0,1.0,1.0);
  auto vel_vec = std::make_shared<Constant>(0.1,0.0,0.0);

  SolutionCation->interpolate(Cation);
  SolutionAnion->interpolate(Anion);
  SolutionPotential->interpolate(Volt);
  // SolutionVelocity->interpolate(*vel_vec);
  SolutionVelocity->interpolate(Velocity);
  SolutionPressure->interpolate(*zero);


  // Previous iteration
  auto PreviousCation = std::make_shared<Function>((*SolutionFunction)[0]);
  auto PreviousAnion = std::make_shared<Function>((*SolutionFunction)[1]);
  auto PreviousPotential = std::make_shared<Function>((*SolutionFunction)[2]);
  auto PreviousVelocity = std::make_shared<Function>((*SolutionFunction)[3]);
  auto PreviousPressure = std::make_shared<Function>((*SolutionFunction)[4]);
  PreviousCation->interpolate(*SolutionCation);
  PreviousAnion->interpolate(*SolutionAnion);
  PreviousPotential->interpolate(*SolutionPotential);
  PreviousVelocity->interpolate(*SolutionVelocity);
  PreviousPressure->interpolate(*SolutionPressure);
  ...
  pnp_stokes_time::BilinearForm a(V,V);
  pnp_stokes_time::LinearForm L(V);
  a.eps = eps; L.eps = eps;
  a.Dp = Dp; L.Dp = Dp;
  a.Dn = Dn; L.Dn = Dn;
  a.qp = qp; L.qp = qp;
  ...
  cationFile << *SolutionCation;
  anionFile << *SolutionAnion;
  potentialFile << *SolutionPotential;
  velocityFile << *SolutionVelocity;
  pressureFile << *SolutionPressure;
  ...
  get_dofs(SolutionFunction.get(), &velocity_dofs, 3);
  get_dofs(SolutionFunction.get(), &pressure_dofs, 4);
  ...
  for (double t = time_step_size; t < final_time; t += time_step_size) {
      ...
      L.CatCat = SolutionCation;
      L.AnAn = SolutionAnion;
      L.EsEs = SolutionPotential;
      L.uu = SolutionVelocity;
      L.pp = SolutionPressure;
      L.CatCat_t0 = PreviousCation;
      L.AnAn_t0 = PreviousAnion;
      assemble(b, L);
      bc1.apply(b);
      bc2.apply(b);
      bc3.apply(b);
      ...
      while (relative_residual > nonlinear_tol && newton_iteration < max_newton_iters )
      {
        ...
        a.CatCat = SolutionCation;
        a.AnAn = SolutionAnion;
        a.EsEs = SolutionPotential;
        a.uu = SolutionVelocity;
        assemble(A, a);
        ...
        *(SolutionCation->vector())+=*(dAnion.vector());
        *(SolutionAnion->vector())+=*(dCation.vector());
        *(SolutionPotential->vector())+=*(dPotential.vector());
        *(SolutionVelocity->vector())+=*(dU.vector());
        *(SolutionPressure->vector())+=*(dPressure.vector());
