...
int main(int argc, char** argv)
{
  ...
  // mesh
  dolfin::Point p0( -Lx/2.0, -Ly/2, -Lz/2);
  dolfin::Point p1(  Lx/2.0,  Ly/2,  Lz/2);
  // auto mesh0 = std::make_shared<dolfin::BoxMesh>(p0, p1, 20, 20, 20);
  auto mesh0 = std::make_shared<dolfin::Mesh>("./benchmarks/battery/cheese.xml");
  // auto mesh0 = std::make_shared<dolfin::Mesh>("./benchmarks/battery/mesh.xml.gz");
...
// meshOut << *mesh0;
  ...
  auto V0 = std::make_shared<pnp::FunctionSpace>(mesh0,periodic_boundary);
  ...
  while (!adaptive_convergence)
  {
    ...
    auto g = std::make_shared<Constant>(0.05);
    ...
    auto boundary = std::make_shared<SymmBoundaries>(0, -Lx/2.0, Lx/2.0);
    dolfin::DirichletBC bc(V, zero_vec, boundary);
    printf("\tComputing fix point..."); fflush(stdout);
    // auto V_index = std::make_shared<pnp::FunctionSpace>(mesh);
    auto one_vec = std::make_shared<Constant>(1.0,1.0,1.0);
    auto Dbd = std::make_shared<DomainBoundary>();
    dolfin::DirichletBC bc_index(V, one_vec, Dbd);
    Function u_index(V);
    ...
    while (flag_index==false)
    {
      if (u_index.vector()->getitem(3*index)!=1){
        // std::cout << "\n toto \t" <<index << '\t' << u_index.vector()->getitem(3*index) << std::endl;
        flag_index=true;
      }
      else
        index+=1;
    ...
    // index=10;
    std::vector<double> dof_coord= V->tabulate_dof_coordinates();
    printf("\tdone. index= %d\n",index); fflush(stdout);
    // std::cout << dof_coord[3*3*index] << '\t' << dof_coord[3*3*index+1]  << '\t' <<  dof_coord[3*3*index+2]  << std::endl;
    // std::cout << dof_coord[3*(3*index+1)] << '\t' << dof_coord[3*(3*index+1)+1]  << '\t' <<  dof_coord[3*(3*index+1)+2]  << std::endl;
    // std::cout << dof_coord[3*(3*index+2)] << '\t' << dof_coord[3*(3*index+2)+1]  << '\t' <<  dof_coord[3*(3*index+2)+2]  << std::endl;
    ...
    while (relative_residual > nonlinear_tol && newton_iteration < max_newton_iters)
    {
      ...
      replace_row(3*index, &A_pnp, &b_pnp);
      replace_row(3*index+1, &A_pnp, &b_pnp);
      replace_row(3*index+2, &A_pnp, &b_pnp);
      // bc.apply(A_pnp);
      // bc.apply(b_pnp);
