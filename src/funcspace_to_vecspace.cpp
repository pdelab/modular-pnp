 ...
 *
 * \brief add A_l the coeffcients of the matrix A_l to the matrix A
...
void add_matrix(int nc, int l, dolfin::FunctionSpace *V, dolfin::FunctionSpace *V_l, dolfin::EigenMatrix *A, dolfin::EigenMatrix *A_l)
{
  ...
  dolfin::la_index rows[nnz];
  dolfin::la_index cols[nnz];
  ...
  for (i=0;i<nrows_l;i++)
  {
    ...
    for (k=IA_l[i];k<IA_l[i+1];k++)
    {
      ...
      rows[k]=(la_index) ii;
      cols[k]=(la_index) jj;
      if ( (ii>nrows) || (jj>ncols) ) printf("==> ERROR\n");
  ...
  // A->add(vals_l, nrows_l, rows, nnz,cols);
